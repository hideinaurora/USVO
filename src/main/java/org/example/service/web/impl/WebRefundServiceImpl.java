...
@Service
@Slf4j
public class WebRefundServiceImpl implements WebRefundService {
    @Resource
    ...
    public PageResult<RefundDetailVO> queryPage(RefundQueryDTO queryDTO) {
        ...
        // 6. 执行分页查询
        Page<RefundEntity> pageResult = refundService.page(page, queryWrapper);
    ...
    public List<RefundDetailVO> queryList() {
        ...
        QueryWrapper<RefundEntity> queryWrapper = new QueryWrapper<>();
        queryWrapper.orderByDesc("gmt_create");
        List<RefundEntity> entityList = refundService.list(queryWrapper);
    ...
    public Boolean examine(RefundExamineDTO examineDTO) {
        ...
        // 1. 查询退款申请
        RefundEntity refundEntity = refundService.getById(examineDTO.getRefundId());
